...
from Pose.datasets.datasets.utils import parse_pose_metainfo
from Pose.models.builder import build_pose_estimator
from Pose.structures import PoseDataSample
from Pose.structures.bbox import bbox_xywh2xyxy
...
def dataset_meta_from_config(config: Config,
    ...
    try:
        ...
        if 'metainfo' in dataset_cfg:
            ...
            import Pose.datasets.datasets  # noqa: F401, F403
            from Pose.registry import DATASETS
...
def init_model(config: Union[str, Path, Config],
    ...
    scope = config.get('default_scope', 'Pose') #! hereee
...
def inference_topdown(model: nn.Module,
    ...
    """
    scope = model.cfg.get('default_scope', 'Pose') #! heree
    if scope is not None:
        init_default_scope(scope)
